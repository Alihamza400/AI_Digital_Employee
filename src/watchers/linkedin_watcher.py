...
class LinkedInWatcher(BaseWatcher):
            ...
            if count > 0:
                ...
                if notif_link:
                    ...
                    items = self.page.query_selector_all(
                        "[data-notification-id], [data-feed-id], article"
                    )
    ...
    def create_action_file(self, item) -> Path:
        ...
        filename = (
            f"LINKEDIN_{item.get('notification_id', item.get('type', 'notif'))}_{timestamp}.md"
        )
...
class LinkedInPoster:
    ...
    def post_content(self, content: str, image_paths: List[str] = None) -> Dict:
        ...
        if not self._session_valid:
            return {
                "success": False,
                "error": "LinkedIn session expired. Call ensure_session() first.",
            }
