...
from .gmail_watcher import GmailSender, GmailWatcher
from .whatsapp_watcher import WhatsAppSender, WhatsAppWatcher
from .linkedin_watcher import LinkedInPoster, LinkedInWatcher, LinkedInPostCreator
...
class ActionRequest:
    ...
    def from_dict(cls, data: Dict) -> "ActionRequest":
        ...
        try:
            at = ActionType(raw_type)
        except (ValueError, KeyError):
            at = ActionType.CREATE_TASK
        data["action_type"] = at
        raw_status = data.get("status", "pending")
        try:
            st = ActionStatus(raw_status)
        except (ValueError, KeyError):
            st = ActionStatus.PENDING
        data["status"] = st
...
class ApprovalWorkflow:
    ...
    def approve(self, request_id: str) -> bool:
        ...
        with self._lock:
            ...
            # Persist the decision, then move to approved folder
            self._move_request(request_id, self.approved_dir, request)
    ...
    def reject(self, request_id: str, reason: str = "") -> bool:
        ...
        with self._lock:
            ...
            # Persist the decision, then move to rejected folder
            self._move_request(request_id, self.rejected_dir, request)

            logger.info(f"Rejected request: {request_id} - {reason}")
            return True

    def _move_request(self, request_id: str, target_dir: Path, request: Optional[Dict] = None):
        """
        Move request file to target directory.

        When the updated request is supplied it is written back before the rename,
        so the archived file records the human decision instead of the stale
        'pending' status it had when it was queued.
        """
...
class ActionExecutor:
    ...
    def execute(self, action: ActionRequest) -> Dict:
        ...
        try:
            ...
            # Handlers signal failure with {'success': False, 'error': ...} instead of
            # raising. Without this check a failed action is recorded as completed and
            # archived to Completed/executed/, corrupting the audit trail.
            if isinstance(result, dict) and result.get("success") is False:
                action.status = ActionStatus.FAILED
                action.error = result.get("error") or "Handler reported failure"
                action.result = result
                logger.error(f"Action {action.id} failed: {action.error}")
                return {"success": False, "error": action.error, "result": result}
    ...
    def _send_whatsapp(self, params: Dict) -> Dict:
        return self.mcp.whatsapp_sender.send_message(
            params.get("phone", ""), params.get("message", "")
        )

    def _post_linkedin(self, params: Dict) -> Dict:
        return self.mcp.linkedin_poster.post_content(
            params.get("content", ""), params.get("images")
        )

    def _create_draft_linkedin(self, params: Dict) -> Dict:
        return self.mcp.linkedin_poster.create_draft(params.get("content", ""))

    def _resolve_vault_path(self, raw_path: Any) -> Optional[Path]:
        """
        Resolve a requested path, confining it to the vault.

        Action parameters originate from LLM-generated approval requests, so an
        unguarded path would let one approved action read or delete arbitrary files
        on the host. The Company Handbook forbids accessing anything "outside
        defined scope", which is exactly the vault.
        """
        if raw_path is None or not str(raw_path).strip():
            return None

        vault_root = self.mcp.vault_path.resolve()
    ...
    def _file_operation(self, params: Dict) -> Dict:
        ...
        if operation not in ("create_file", "delete_file"):
            return {"success": False, "error": f"Unknown operation: {operation}"}

        path = self._resolve_vault_path(params.get("path"))
        if path is None:
            return {
                "success": False,
                "error": "A 'path' pointing inside the vault is required for file operations",
            }
...
"""
    ...
    def _create_invoice(self, params: Dict) -> Dict:
        ...
        pdf.cell(
            0, 6, f'Date: {datetime.now().strftime("%Y-%m-%d")}', new_x="LMARGIN", new_y="NEXT"
        )
        pdf.ln(6)
        col_w = (190 - 40) / 4
...
class MCPServer:
    ...
    def __init__(self, vault_path: str, config: Dict[str, Any] = None):
        ...
        # Action history. The log file is read-modify-written by several threads
        # (approval watcher, scheduler, main loop), so every write is serialized.
        self._log_lock = Lock()
    ...
    def _save_action(self, action: ActionRequest):
        """
        Append an action to today's history log.

        Serialized with a lock: a concurrent read-modify-write would drop entries
        and corrupt the audit trail this system is built around.
        """
    ...
    def gmail_watcher(self):
        if self._gmail_watcher is None:
            client_config = self.config.get("gmail_client_config", {})
            token_data = self.config.get("gmail_token_json", {})
            self._gmail_watcher = GmailWatcher(self.vault_path, client_config, token_data)
    ...
    def create_action(
        self, action_type: ActionType, parameters: Dict, requires_approval: bool = True
    ) -> ActionRequest:
    ...
    def send_whatsapp(
        self, phone: str, message: str, requires_approval: bool = True
    ) -> ActionRequest:
        """Create WhatsApp send action"""
        return self.create_action(
            ActionType.SEND_WHATSAPP, {"phone": phone, "message": message}, requires_approval
        )

    def post_linkedin(
        self, content: str, images: List[str] = None, requires_approval: bool = True
    ) -> ActionRequest:
    ...
    def create_linkedin_draft(self, content: str) -> ActionRequest:
        ...
        return self.create_action(
            ActionType.CREATE_DRAFT_LINKEDIN, {"content": content}, requires_approval=False
        )
    ...
    def get_status(self) -> Dict:
        ...
        Service flags describe configuration, not whether the lazily-created client
        object happens to exist yet — those are only built on first use, so reading
        them here would always report 'not configured'.
        """
        return {
            ...
            "gmail_configured": bool(
                self.config.get("gmail_client_config") and self.config.get("gmail_token_json")
            ),
            "calendar_configured": bool(
                self.config.get("calendar_client_config") and self.config.get("calendar_token_json")
            ),
            "whatsapp_configured": bool(self.config.get("whatsapp_session")),
            "linkedin_configured": bool(self.config.get("linkedin_session")),
