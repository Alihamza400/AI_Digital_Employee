...
class CronScheduler:
    ...
    def _add_job(
        self, task_id: str, name: str, func: callable, trigger: Any, args: tuple, kwargs: dict
    ) -> str:
        """
        Add a job to the scheduler.

        The APScheduler callback is _job_wrapper rather than func itself so that
        last_run / run_count / last_error are recorded and the per-task `enabled`
        flag (set by pause_job) is honored. Without the wrapper the status report
        always claimed a job had never run.
        """
    ...
    def run_job_now(self, task_id: str) -> bool:
        """Run a scheduled job immediately (used for smoke tests and manual triggers)"""
        task = self.tasks.get(task_id)
        if not task:
            return False
        self._job_wrapper(task_id, task.func, task.args, task.kwargs)
        return True
...
class AutomatedTasks:
    ...
    def __init__(self, vault_path: str, mcp_server=None, scheduler=None):
        self.vault_path = Path(vault_path)
        self.mcp = mcp_server
...
"""
    ...
    def update_dashboard(self):
        ...
        # Count files in each vault folder. Completed/ archives live in status
        # subfolders (executed/, failed/, rejected/), so it must count recursively.
        def count_files(subdir: str, recursive: bool = False) -> int:
            d = self.vault_path / subdir
            if not d.exists():
                return 0
            entries = d.rglob("*") if recursive else d.iterdir()
            return len([f for f in entries if f.is_file()])
        ...
        completed = count_files("Completed", recursive=True)

        job_count = len(self.scheduler.tasks) if self.scheduler else None
        scheduler_status = f"✅ RUNNING ({job_count} jobs)" if job_count else "✅ RUNNING"
...
class ScheduledTaskManager:
    ...
    def __init__(self, vault_path: str, mcp_server):
        ...
        self.tasks = AutomatedTasks(vault_path, mcp_server, self.scheduler)
    ...
    def _setup_default_schedule(self):
        ...
        self.scheduler.add_interval_job(
            "check_approvals", "Check Pending Approvals", self._check_approvals, 300  # 5 minutes
        )

        # Update dashboard every 5 minutes
        self.scheduler.add_interval_job(
            "update_dashboard", "Update Dashboard.md", self._update_dashboard, 300  # 5 minutes
        )
